...
describe("Riverbend deterministic policy", () => {
  ...
  it("classifies exactly three years as follow-up and older records as new", () => {
    expect(classifyVisit({ ...patient("PAT-0102"), lastSeen: "2023-08-26" })).toBe("follow_up");
    expect(classifyVisit({ ...patient("PAT-0102"), lastSeen: "2023-08-25" })).toBe("new_patient");
  ...
  it("blocks inactive coverage before availability", () => {
    ...
    expect(decision).toMatchObject({ allowed: false, reasonCode: "INACTIVE_INSURANCE" });
    expect(searchEligibleSlots(decision)).toHaveLength(0);
  });

  it("blocks a missing policy number or card without inventing a value", () => {
    const decision = evaluateBooking(patient("PAT-0111"), false);
    expect(decision).toMatchObject({ allowed: false, reasonCode: "POLICY_NUMBER_REQUIRED" });
  });
  ...
  it("requires office approval for a different follow-up provider", () => {
    expect(evaluateBooking(patient("PAT-0102"), true, "PROV-WHITFIELD")).toMatchObject({
      allowed: false,
  ...
  it("keeps Dr. Crane on Thursday and allows the paired NP when accepted", () => {
    ...
    const mendez = evaluateBooking(patient("PAT-0107"), true, "PROV-MENDEZ", true);
    expect(searchEligibleSlots(mendez, [], "PROV-MENDEZ")[0].id).toBe("SLOT-002");
  ...
  it("offers the chronologically earliest eligible slot", () => {
    ...
    expect(eligible.every((slot, index) => index === 0 || eligible[index - 1].start <= slot.start)).toBe(true);
  });
});

describe("Conversation safety and privacy", () => {
  it("preempts ordinary flow for a possible emergency and never looks up a patient", () => {
    const result = processTurn(createSession(), "I am passing out and cannot breathe");
    expect(result.outcome).toBe("emergency_instruction");
    expect(result.reply).toContain("dial 911");
    expect(result.session.trace.some((item) => item.ruleId === "RB-SAFE-01")).toBe(true);
    expect(result.session.trace.some((item) => item.tool === "lookupPatient")).toBe(false);
  });

  it("routes urgent clinical language without medical advice", () => {
    const result = processTurn(createSession(), "I have severe pain and need to know what to do");
    expect(result.outcome).toBe("transfer_nurse_line");
    expect(result.reply).toContain("can’t provide medical advice");
    expect(result.session.trace.some((item) => item.tool === "transferCall")).toBe(true);
  });

  it("does not disclose appointment data after a DOB mismatch", () => {
    let session = processTurn(createSession(), "Confirm my appointment").session;
    ...
    expect(result.session.trace.some((item) => item.tool === "getAppointments")).toBe(false);
  });

  it("never disambiguates duplicate names by name alone", () => {
    let session = processTurn(createSession(), "I need to book an appointment").session;
...
describe("Mutation confirmation", () => {
  it("does not book until an explicit confirmation turn", () => {
    ...
    for (const turn of ["Book an appointment", "555-0102", "1971-07-04", "yes"]) {
    ...
    expect(session.trace.some((item) => item.tool === "bookAppointment")).toBe(false);

    const result = processTurn(session, "yes");
    expect(result.outcome).toBe("booked");
    expect(result.session.appointments).toHaveLength(4);
    expect(result.session.trace.some((item) => item.tool === "bookAppointment")).toBe(true);
  });
  ...
  it("stops a cancellation when the caller does not confirm", () => {
    ...
    expect(result.session.appointments.find((item) => item.id === "APT-1002")?.status).toBe("booked");
  });
  ...
  it("cancels only the selected appointment after confirmation", () => {
    ...
    expect(result.session.appointments.find((item) => item.id === "APT-1002")?.status).toBe("cancelled");
    expect(result.session.appointments.find((item) => item.id === "APT-1001")?.status).toBe("booked");
  });
});

describe("Scenario catalog", () => {
  it("contains every required Phase 1 scenario family", () => {
    expect(new Set(scenarios.map((item) => item.family))).toEqual(new Set(["booking", "appointment", "identity", "faq", "transfer", "safety"]));
    expect(scenarios).toHaveLength(24);
  });

  it("replays all 24 catalog scenarios successfully", () => {
    const failures = scenarios.map((scenario) => ({ id: scenario.id, ...replayScenario(scenario.id) })).filter((result) => !result.passed);
