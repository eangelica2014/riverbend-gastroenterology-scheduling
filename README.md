...
| **Voice/text agent**       | Adds a live transcript and evidence-derived progress rail while booking, rescheduling, cancelling, confirming, answering approved FAQs, and transferring | `/agent`                                                                  |
...
The browser speech APIs are a progressive enhancement. The complete proof remains operable through text and scripted scenarios when microphone permission or browser support is unavailable. Interim recognition appears in a local-session transcript so a caller can notice a misheard phrase; the transcript never substitutes for identity verification. A seven-stage progress rail derives from the same session and trace evidence used by the FDE—intent, identity, coverage, policy, availability, confirmation, and outcome—rather than from an unrelated animation.

The interactive presentation includes a dedicated GitHub CI/CD evidence slide. It shows the release contract used by the repository: locked dependency installation, deterministic assertions, catalog scenario replay, TypeScript validation, and production build. It also preserves the actual failed-to-green workflow iteration instead of presenting release quality as a frictionless claim.
